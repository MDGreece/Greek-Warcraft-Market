...
Run node scripts/update-warcraftlogs.js
  node scripts/update-warcraftlogs.js
  shell: /usr/bin/bash -e {0}
  env:
    WARCRAFTLOGS_CLIENT_ID: ***
    WARCRAFTLOGS_CLIENT_SECRET: ***

That means the script ran but printed nothing. Most likely update-warcraftlogs.js is still empty or only partly saved.

Open scripts/update-warcraftlogs.js and check:

The first line must be:

const fs = require("fs");
And near the bottom it must contain:

console.log(`Fetching Warcraft Logs for ${group.name}...`);
If not, paste the script again and Commit changes.

Quick test: add this as the first line of run():

console.log("Warcraft Logs script started");
Run workflow again. If you don’t see that line, GitHub is not using the file you edited.


My only line in update warcraft logs is
const fs = require("fs");

That’s why nothing happens. Replace the whole file with this:
...
Group II: found latest report, counted 9 pulls, best pull 52.71%

Group III: latest report had 0 encounter fights, so it shows 0 and -

The issue is that the script always uses the latest report, but sometimes the latest report is empty/bad.

Next fix: make the script scan the latest 20 reports and pick the first one with boss pulls.

Replace this line:

group.latestReport = reports[0]?.code || "";
with logic that checks reports one by one.

I’ll give you the corrected full script next.
...
That is your inspect script, not the real updater.
...
scripts/update-warcraftlogs.js
Replace that file with this full version:
...
async function run() {
  ...
  for (const group of groups) {
    ...
    for (const report of reports) {
      const progress = await getReportProgress(token, report.code);
    ...
    if (selectedReport && selectedProgress) {
      group.latestReport = selectedReport.code;
      group.latestReportTitle = selectedReport.title || "";
      group.totalPulls = selectedProgress.totalPulls;
      group.bossProg = selectedProgress.bossProg;
      group.bestBoss = selectedProgress.bestBoss;
    } else {
      group.latestReport = "";
